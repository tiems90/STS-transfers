...
cargos_indexed = spark.read.table('ship2ship.cargos_indexed').repartition(sc.defaultParallelism * 20).filter(col('timestamp').between('2018-01-31T00:00:00.000+0000','2018-01-31T23:59:00.000+0000'))
...
lines = (cargos_indexed
  .repartition(sc.defaultParallelism * 20)
  .groupBy("mmsi", window("timestamp", "15 minutes"))
  .agg(
    collect_list(struct(col("point_geom"), col("timestamp")))
      .alias("coords")
  )
  .withColumn("coords", expr("""
  ...
  .withColumn("line", st_makeline(col("coords.point_geom")))
  .cache()
)
...
def get_buffer(line):
  ...
  return lit(max_np) * lit(buffer) / np # inverse proportional to number of points, larger buffer for slower ships

cargo_movement = (
  lines
    .withColumn("buffer_r", get_buffer("line"))
    .withColumn("buffer_geom", st_buffer('line', col("buffer_r")))
    .withColumn('buffer', st_astext('buffer_geom'))
...
candidates_lines = (
  ...
  .join(
    ...
    (col('a.ix.is_core') |  col('b.ix.is_core')) | st_intersects('a.ix.wkb', 'b.ix.wkb')
  )
   .select(
...
matches = candidates_lines.join(
  harbours_h3, how="leftouter", on=candidates_lines["index"] == harbours_h3["h3"]
).where(
  harbours_h3["h3"].isNull()
).groupBy("vessel_1", "vessel_2").agg(
  first("line_1").alias("line_1"),
  first("line_2").alias("line_2")
)
